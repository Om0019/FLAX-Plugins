...
const AIOSTREAMS_PASSWORD = ((typeof process !== "undefined" && process.env ? process.env.AIOSTREAMS_PASSWORD : undefined));
...
function finalizeStreams(streams) {
  return streams.filter(function (s) { return s != null; }).map((stream, index) => ({ stream, index })).sort((a, b) => {
