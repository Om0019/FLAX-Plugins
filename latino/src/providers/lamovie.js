/**
 * LaMovie provider for Nuvio Local Scrapers.
 *
 * Ported from the "Latino" Stremio addon (src/scrapers/lamovie.js +
 ...
 *
 * LaMovie itself is a JSON API (search/episodes/player), so it needs none of
 * cheerio's HTML parsing. The generic player unpacker below (shared verbatim
 * with providers/sololatino.js, providers/cuevana3i.js, providers/cinecalidad.js
 * and providers/tioplus.js, since Nuvio's sandbox can't `require('../unpacker')`
 * across files) still applies to resolving each embed it returns, and stubs
 * out three resolvers that depend on Node's `crypto` module for real
 * cryptography (AES-GCM / AES-CBC / SHA-256) not reasonably reimplementable
 * by hand here:
 ...
 * Everything else from the original unpacker — Dean Edwards
 * `eval(function(p,a,c,k,e,d)` unpacking, VOE, Dood, Streamtape, Nupload,
 * MediaFire, VidGuard, Xupalace multi-server pages, JS redirects and iframe
 * chasing — is ported faithfully.
 */
...
function fetchTmdbDetails(tmdbId, mediaType) {
  ...
  return fetchFromTmdb(`/${tmdbType}/${tmdbId}`, { language: 'es-MX' })
    ...
    .catch((error) => {
      console.error(`LaMovie: TMDB lookup failed for ${mediaType}/${tmdbId}:`, error.message);
      return null;
    });
}
...
function* iterUnpackedScripts(html) {
  ...
  while ((match = packerRegex.exec(html || '')) !== null) {
    try {
      ...
      console.error('LaMovie unpacker: failed to decode script block:', err.message);
    }
  }
}
...
function resolveXupalaceServers(html, baseUrl, userAgent, options) {
  ...
  return firstResultInOrder(servers, EMBED_RESOLVE_CONCURRENCY, (entry) =>
    resolvePlayerStream(entry.url, userAgent, baseUrl, { depth: depth + 1, visited, signal }).catch((e) => {
      console.warn(`LaMovie unpacker: Xupalace server ${entry.server || entry.url} failed: ${e.message}`);
      return null;
    })
  );
}
...
function decodeVoePayload(encoded, options = {}) {
  try {
    ...
    if (!options.quiet) console.warn(`LaMovie unpacker: VOE payload decode failed: ${error.message}`);
    return null;
  }
}
...
function extractNuploadDirectStream(html, baseUrl) {
  ...
  try {
    ...
    console.warn(`LaMovie unpacker: Nupload decode failed: ${error.message}`);
  }
  return null;
}
...
function decodeVidguardSignature(streamUrl) {
  try {
    ...
    console.warn(`LaMovie unpacker: VidGuard signature decode failed: ${error.message}`);
    return streamUrl;
  }
}
...
function solveEmbed69ProofOfWork(challenge, difficulty) {
  ...
  function step() {
    ...
    if (Date.now() > deadline) {
      console.warn(`LaMovie: embed69 proof-of-work (difficulty ${difficulty}) exceeded ${EMBED69_MAX_POW_MS}ms after ${nonce} nonces; giving up.`);
      return null;
    }
...
function decryptEmbed69(html) {
  ...
  if (difficulty > EMBED69_MAX_POW_DIFFICULTY) {
    console.warn(`LaMovie: refusing embed69 proof-of-work at difficulty ${difficulty} (max ${EMBED69_MAX_POW_DIFFICULTY} in this pure-JS sandbox).`);
    return Promise.resolve(null);
  }
...
function resolvePlayerStream(url, userAgent, referer, options = {}) {
  ...
  if (isDefunctHost(url)) {
    console.log(`LaMovie unpacker: skipping ${getHostname(url)}, which accepts no connections`);
    return Promise.resolve(null);
  }
  ...
  return step
    ...
    .catch((e) => {
      console.warn(`LaMovie unpacker: player wrapper skipped (${getHostname(url) || url}): ${e.message}`);
      return null;
    });
}
...
function resolveFromPage(url, html, res, userAgent, referer, depth, visited, signal) {
  ...
  chain = chain.then((result) => {
    ...
    if (!isVoeHost(url)) {
      ...
      if (voeMirrorUrl) {
        console.log(`LaMovie unpacker: recognised ${getHostname(url)} as a VOE mirror by payload`);
        return voeMirrorUrl;
      }
    }
    return null;
  ...
  chain = chain.then((result) => {
    ...
    if (redirectUrl && redirectUrl !== url && isHttpUrl(redirectUrl)) {
      console.log(`LaMovie unpacker: following JS redirect to ${redirectUrl}`);
  ...
  chain = chain.then((result) => {
    ...
    if (assignedRedirectUrl && assignedRedirectUrl !== url && isHttpUrl(assignedRedirectUrl)) {
      console.log(`LaMovie unpacker: following assigned redirect to ${assignedRedirectUrl}`);
...
// ---------------------------------------------------------------------------
// LaMovie-specific scraping (ported from src/scrapers/lamovie.js)
// ---------------------------------------------------------------------------

const LAMOVIE_BASE_URL = 'https://lamovie.org';
const LAMOVIE_API_URL = `${LAMOVIE_BASE_URL}/wp-api/v1`;
const LAMOVIE_SEARCH_TIMEOUT_MS = 8000;
const LAMOVIE_PLAYER_TIMEOUT_MS = 5500;
const LAMOVIE_EPISODES_TIMEOUT_MS = 5000;
const LAMOVIE_PLAYER_CONCURRENCY = 4;
...
function scoreCandidate(result, title, originalTitle, year, type, extraTitles = []) {
  ...
  if (cleanOriginal && (looseIncludes(cleanOriginalResult, cleanOriginal) || looseIncludes(cleanSlug, cleanOriginal))) score += 4;
  ...
  for (const cleanExtra of cleanExtras) {
    if (looseIncludes(cleanResult, cleanExtra) || looseIncludes(cleanSlug, cleanExtra) || looseIncludes(cleanOriginalResult, cleanExtra)) score += 4;
 ...
 */
function lamovieSearch(title, originalTitle, year, type, userAgent, signal, extraTitles = []) {
  const queries = [...new Set([title, originalTitle, ...extraTitles].filter(Boolean))];
  const runQuery = (query) => runLaMovieQuery(query, title, originalTitle, year, type, userAgent, signal, extraTitles);
...
function scrape(title, originalTitle, year, type, season, episode, options = {}) {
  ...
  return lamovieSearch(title, originalTitle, year, type, userAgent, signal, extraTitles)
    .then((match) => {
      if (!match) {
      ...
      const postIdPromise = type === 'series'
        ? getEpisodePostId(match._id, season, episode, userAgent, signal).then((postId) => {
            if (!postId) console.log(`LaMovie: No episode found for "${title}" S${season}E${episode}`);
            return postId;
          })
      ...
      return postIdPromise.then((postId) => {
        ...
        return fetchJsonWithTimeout(playerUrl, {
          ...
          return mapWithConcurrency(embeds, LAMOVIE_PLAYER_CONCURRENCY, (embed, index) => {
            const embedUrl = embed.url;
            if (!embedUrl || /^magnet:/i.test(embedUrl)) return null;

            return resolvePlayerStream(embedUrl, userAgent, LAMOVIE_BASE_URL, { signal }).then((resolvedUrl) => {
  ...
  return Promise.all([fetchTmdbDetails(tmdbId, mediaType), getAlternativeTitles(mediaType, tmdbId)])
    .then(([details, extraTitles]) => {
      ...
      return scrape(details.title, details.originalTitle, details.year, type, seasonNum, episodeNum, { extraTitles }).then((results) => {
        ...
        return mapWithConcurrency(rawNuvioStreams.slice(0, 10), STREAM_PROBE_CONCURRENCY,
