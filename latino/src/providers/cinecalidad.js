...
function waitForPlayerResolvers(playerPromises, streams) {
  return new Promise((resolve) => {
    ...
    // Without timers there is no grace period and no collection deadline to
    // fire, so the fast path never arms and this just waits for every player
    // promise to settle below.
    safeSetTimeout(() => {
