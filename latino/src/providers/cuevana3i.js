/**
 * Cuevana3i provider for Nuvio Local Scrapers.
 *
 * Ported from the "Latino" Stremio addon (src/scrapers/cuevana3i.js +
 ...
 * since Nuvio's sandbox can't `require('../unpacker')` across files) stubs out
 * three resolvers that depend on Node's `crypto` module for real cryptography
 * (AES-GCM / AES-CBC / SHA-256) not reasonably reimplementable by hand here:
 ...
 * Cuevana3i's own wrapper-URL decoding (XOR + base64, see decodeWrapperUrl
 * below) needs none of that — it ports without any stubbing. Everything else
 * from the original unpacker — Dean Edwards `eval(function(p,a,c,k,e,d)`
...
function fetchTmdbDetails(tmdbId, mediaType) {
  ...
  return fetchFromTmdb(`/${tmdbType}/${tmdbId}`, { language: 'es-MX' })
    ...
    .catch((error) => {
      console.error(`Cuevana3i: TMDB lookup failed for ${mediaType}/${tmdbId}:`, error.message);
      return null;
    });
}
...
function* iterUnpackedScripts(html) {
  ...
  while ((match = packerRegex.exec(html || '')) !== null) {
    try {
      ...
      console.error('Cuevana3i unpacker: failed to decode script block:', err.message);
    }
  }
}
...
function resolveXupalaceServers(html, baseUrl, userAgent, options) {
  ...
  return firstResultInOrder(servers, EMBED_RESOLVE_CONCURRENCY, (entry) =>
    resolvePlayerStream(entry.url, userAgent, baseUrl, { depth: depth + 1, visited, signal }).catch((e) => {
      console.warn(`Cuevana3i unpacker: Xupalace server ${entry.server || entry.url} failed: ${e.message}`);
      return null;
    })
  );
}
...
function decodeVoePayload(encoded, options = {}) {
  try {
    ...
    if (!options.quiet) console.warn(`Cuevana3i unpacker: VOE payload decode failed: ${error.message}`);
    return null;
  }
}
...
function extractNuploadDirectStream(html, baseUrl) {
  ...
  try {
    ...
    console.warn(`Cuevana3i unpacker: Nupload decode failed: ${error.message}`);
  }
  return null;
}
...
function decodeVidguardSignature(streamUrl) {
  try {
    ...
    console.warn(`Cuevana3i unpacker: VidGuard signature decode failed: ${error.message}`);
    return streamUrl;
  }
}
...
function solveEmbed69ProofOfWork(challenge, difficulty) {
  ...
  function step() {
    ...
    if (Date.now() > deadline) {
      console.warn(`Cuevana3i: embed69 proof-of-work (difficulty ${difficulty}) exceeded ${EMBED69_MAX_POW_MS}ms after ${nonce} nonces; giving up.`);
      return null;
    }
...
function decryptEmbed69(html) {
  ...
  if (difficulty > EMBED69_MAX_POW_DIFFICULTY) {
    console.warn(`Cuevana3i: refusing embed69 proof-of-work at difficulty ${difficulty} (max ${EMBED69_MAX_POW_DIFFICULTY} in this pure-JS sandbox).`);
    return Promise.resolve(null);
  }
...
function resolvePlayerStream(url, userAgent, referer, options = {}) {
  ...
  if (isDefunctHost(url)) {
    console.log(`Cuevana3i unpacker: skipping ${getHostname(url)}, which accepts no connections`);
    return Promise.resolve(null);
  }
  ...
  return step
    ...
    .catch((e) => {
      console.warn(`Cuevana3i unpacker: player wrapper skipped (${getHostname(url) || url}): ${e.message}`);
      return null;
    });
}
...
function resolveFromPage(url, html, res, userAgent, referer, depth, visited, signal) {
  ...
  chain = chain.then((result) => {
    ...
    if (!isVoeHost(url)) {
      ...
      if (voeMirrorUrl) {
        console.log(`Cuevana3i unpacker: recognised ${getHostname(url)} as a VOE mirror by payload`);
        return voeMirrorUrl;
      }
    }
    return null;
  ...
  chain = chain.then((result) => {
    ...
    if (redirectUrl && redirectUrl !== url && isHttpUrl(redirectUrl)) {
      console.log(`Cuevana3i unpacker: following JS redirect to ${redirectUrl}`);
  ...
  chain = chain.then((result) => {
    ...
    if (assignedRedirectUrl && assignedRedirectUrl !== url && isHttpUrl(assignedRedirectUrl)) {
      console.log(`Cuevana3i unpacker: following assigned redirect to ${assignedRedirectUrl}`);
...
// ---------------------------------------------------------------------------
// Cuevana3i-specific scraping (ported from src/scrapers/cuevana3i.js)
// ---------------------------------------------------------------------------

const PLAYER_CONCURRENCY = 5;
const PLAYER_RESOLVE_TIMEOUT_MS = 1800;
const CUEVANA_PAGE_TIMEOUT_MS = 5000;
const CUEVANA_PROBE_TIMEOUT_MS = 2500;
...
function resolveWithTimeout(url, userAgent, referer, signal) {
  return new Promise((resolve) => {
    let timer;
    let settled = false;
    ...
    timer = safeSetTimeout(() => finish(null), PLAYER_RESOLVE_TIMEOUT_MS);

    resolvePlayerStream(url, userAgent, referer, { signal })
      .then(finish)
      .catch(() => finish(null));
  });
}
...
function findOptionLabel(pageDoc, wrapperUrl) {
  let label = '';
  pageDoc('[data-url], [data-server]').each((_, el) => {
    if (label) return;
    if (pageDoc(el).attr('data-url') === wrapperUrl || pageDoc(el).attr('data-server') === wrapperUrl) {
      // `data-server` elements wrap two spans (server name, then a "Reproducir"
      // button label) — take just the first so the label isn't "Servidor HyperReproducir".
      const firstSpan = pageDoc(el).find('span').first();
      label = (firstSpan.length ? firstSpan.text() : pageDoc(el).text()).trim();
    }
  });
  return label;
}
...
function extractWrapperUrls(html) {
  // The wrapper subdomain's TLD has moved at least once in the wild (`.you` ->
  // `.bid`), so this matches any TLD rather than pinning to the one seen when
  // this scraper was last verified against the live site.
  const matches = html.match(/https:\/\/tungtungsahur\.cuevana3i\.[a-z]+\/\?(?:token|v)=[^"'`\s<>]+/g) || [];
  ...
  try {
    ...
    if (token) {
      ...
      if (baseUrl) {
        ...
        const decoded = base64ToBinaryString(token.slice(1));
        let decrypted = '';
    ...
    if (v) {
      const decoded = base64ToUtf8String(v).trim();
...
function probePage(candidate, userAgent, signal) {
  return fetchWithTimeout(candidate.url, { headers: { 'User-Agent': userAgent }, signal }, CUEVANA_PROBE_TIMEOUT_MS)
...
function scrape(title, originalTitle, year, type, season, episode, options = {}) {
  ...
  return firstResultInOrder(candidates, PROBE_CONCURRENCY, (candidate) => probePage(candidate, userAgent, signal))
    .then((bestMatch) => {
      if (bestMatch) {
      ...
      return fetchTextWithTimeout(targetPageUrl, { headers: { 'User-Agent': userAgent }, signal }, CUEVANA_PAGE_TIMEOUT_MS)
        .then(({ res: pageRes, text: pageHtml }) => {
          ...
          return mapWithConcurrency(wrapperUrls, PLAYER_CONCURRENCY, (wrapperUrl, index) => {
            const decodedUrl = decodeWrapperUrl(wrapperUrl);
            const optionName = findOptionLabel(pageDoc, wrapperUrl) || `Opcion ${index + 1}`;

            if (!decodedUrl) return null;

            return resolveWithTimeout(decodedUrl, userAgent, targetPageUrl, signal)
              .then((directUrl) => {
                if (!directUrl) return null;
                return {
                  name: 'Cuevana3i',
                  title: `🇲🇽 ${optionName}`,
                  url: directUrl,
                  headers: { 'User-Agent': userAgent, Referer: decodedUrl || targetPageUrl }
                };
              })
              .catch((err) => {
...
function getStreams(tmdbId, mediaType, seasonNum, episodeNum) {
  ...
  return Promise.all([fetchTmdbDetails(tmdbId, mediaType), getAlternativeTitles(mediaType, tmdbId)])
    ...
    .catch((error) => {
      console.error('Cuevana3i (Nuvio): getStreams failed:', error && error.message);
      return [];
    });
}
