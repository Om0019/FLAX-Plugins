/**
 * Ennovelas provider for Nuvio Local Scrapers.
 *
 * Ported from the "Latino" Stremio addon (src/scrapers/ennovelas.js +
 ...
 * Ennovelas's own episode lookup, `novel.php?post=` base64 wrapper decoding
 * and player-URL normalization need none of that — they port without any
 * stubbing. Everything else from the original unpacker — Dean Edwards
...
// ---------------------------------------------------------------------------
// Ennovelas-specific scraping (ported from src/scrapers/ennovelas.js)
// ---------------------------------------------------------------------------
...
function fetchPageOk(url, userAgent, signal, episode) {
  return fetchTextWithTimeout(url, { headers: ennovelasBrowserHeaders(userAgent), signal }, ENNOVELAS_PAGE_TIMEOUT_MS)
    .then(({ res, text }) => {
...
function findEpisodeUrl(title, originalTitle, season, episode, userAgent, signal, extraTitles = []) {
  ...
  function runQuery(query) {
    ...
    return fetchTextWithTimeout(searchUrl, { headers: ennovelasBrowserHeaders(userAgent), signal }, ENNOVELAS_SEARCH_TIMEOUT_MS)
      .then(({ res, text: html }) => {
        if (!res.ok) return null;
  ...
  return findEpisodeUrl(title, originalTitle, season, episode, userAgent, signal, extraTitles)
    .then((episodeMatch) => {
      ...
      const episodeHtmlPromise = episodeMatch.html
        ...
        : fetchPageOk(episodeMatch.url, userAgent, signal, episode);
      ...
      return episodeHtmlPromise.then((episodeHtml) => {
        ...
        return mapWithConcurrency(playerUrls, ENNOVELAS_PLAYER_CONCURRENCY, (playerUrl) =>
...
/**
 * Required Nuvio local-scraper entry point. Ennovelas only serves
