// Built from src/providers/pelispedia.js for the Hermes/es2016 runtime Nuvio's local-scraper
// sandbox targets -- do not hand-edit. Regenerate with:
//   npx esbuild@0.28.1 --target=es2016 --format=cjs --platform=neutral src/providers/pelispedia.js > latino/providers/pelispedia.js
// Edit src/providers/pelispedia.js instead, then rebuild.
...
const PELISPEDIA_SEARCH_TIMEOUT_MS = 5e3;
...
function pelispediaSlugifyTitle(str) {
  if (!str) return "";
  return str.toLowerCase().normalize("NFD").replace(/[̀-ͯ]/g, "").replace(/&/g, " and ").replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
}
function scoreCandidate(result, title, originalTitle, year, extraTitles = []) {
  ...
  const cleanSlug = cleanText((_a = result.url.split("/").pop()) == null ? void 0 : _a.replace(/-/g, " "));
  let score = 0;
...
function runPelispediaQuery(query, title, originalTitle, year, type, userAgent, signal, extraTitles) {
  ...
  return fetchTextWithTimeout(searchUrl, { headers: { "User-Agent": userAgent }, signal }, PELISPEDIA_SEARCH_TIMEOUT_MS).then(({ res, text: html }) => {
    if (!res.ok) return null;
    const $ = cheerio.load(html);
    const results = [];
    $("a[href]").each((_, el) => {
...
function pelispediaSearch(title, originalTitle, year, type, userAgent, signal, extraTitles = []) {
  ...
  return raceTitleSearches(queries.slice(0, 2), runQuery).then((racedMatch) => {
    ...
    function tryFallback(i) {
      ...
      return fetchWithTimeout(candidateUrl, { headers: { "User-Agent": userAgent }, signal }, PELISPEDIA_SEARCH_TIMEOUT_MS).then((res) => res.ok ? candidateUrl : tryFallback(i + 1)).catch(() => tryFallback(i + 1));
    }
...
function findEpisodeUrl(html, pageUrl, season, episode) {
  ...
  const $ = cheerio.load(html);
  const patterns = [
    new RegExp(`(?:temporada|season)[^0-9]*${season}[^0-9]+(?:episodio|episode|capitulo|cap\xEDtulo)[^0-9]*${episode}`, "i"),
...
function scrape(title, originalTitle, year, type, season, episode, options = {}) {
  ...
  return pelispediaSearch(title, originalTitle, year, type, userAgent, signal, extraTitles).then((pageUrl) => {
    if (!pageUrl) {
      console.log(`PelisPedia: No matching content found for "${title}"`);
      return [];
    }
    return fetchTextWithTimeout(pageUrl, { headers: { "User-Agent": userAgent }, signal }, PELISPEDIA_PAGE_TIMEOUT_MS).then((page) => {
      ...
      const moviePagePromise = type === "series" ? (() => {
        ...
        return fetchTextWithTimeout(episodeUrl, { headers: { "User-Agent": userAgent }, signal }, PELISPEDIA_PAGE_TIMEOUT_MS).then((episodePage) => episodePage.res.ok ? { targetUrl: episodeUrl, html: episodePage.text } : null);
      ...
      return moviePagePromise.then((resolved) => {
        ...
        return mapWithConcurrency(
          iframeUrls,
          PELISPEDIA_PLAYER_CONCURRENCY,
          (iframeUrl, index) => resolvePlayerStream(iframeUrl, userAgent, targetUrl, { signal }).then((resolvedUrl) => {
            if (!resolvedUrl) return null;
            return {
              name: "PelisPedia",
              title: `\u{1F1F2}\u{1F1FD} Opcion ${index + 1}`,
              url: resolvedUrl,
