...
function fetchTmdbDetails(tmdbId, mediaType) {
  ...
  return fetchFromTmdb(`/${tmdbType}/${tmdbId}`, { language: "es-MX" }).then((data) => {
    ...
    console.error(`Ennovelas: TMDB lookup failed for ${mediaType}/${tmdbId}:`, error.message);
    return null;
  });
}
...
function* iterUnpackedScripts(html) {
  ...
  while ((match = packerRegex.exec(html || "")) !== null) {
    try {
      ...
      console.error("Ennovelas unpacker: failed to decode script block:", err.message);
    }
  }
}
...
function resolveXupalaceServers(html, baseUrl, userAgent, options) {
  ...
  return firstResultInOrder(
    ...
    (entry) => resolvePlayerStream(entry.url, userAgent, baseUrl, { depth: depth + 1, visited, signal }).catch((e) => {
      console.warn(`Ennovelas unpacker: Xupalace server ${entry.server || entry.url} failed: ${e.message}`);
      return null;
    })
  );
}
...
function decodeVoePayload(encoded, options = {}) {
  try {
    ...
    if (!options.quiet) console.warn(`Ennovelas unpacker: VOE payload decode failed: ${error.message}`);
    return null;
  }
}
...
function extractNuploadDirectStream(html, baseUrl) {
  ...
  try {
    ...
    console.warn(`Ennovelas unpacker: Nupload decode failed: ${error.message}`);
  }
  return null;
}
function decodeVidguardSignature(streamUrl) {
  try {
    ...
    console.warn(`Ennovelas unpacker: VidGuard signature decode failed: ${error.message}`);
    return streamUrl;
  }
}
...
function resolvePlayerStream(url, userAgent, referer, options = {}) {
  ...
  if (isDefunctHost(url)) {
    console.log(`Ennovelas unpacker: skipping ${getHostname(url)}, which accepts no connections`);
    return Promise.resolve(null);
  }
  ...
  return step.then((result) => {
    ...
    console.warn(`Ennovelas unpacker: player wrapper skipped (${getHostname(url) || url}): ${e.message}`);
    return null;
  });
}
function resolveFromPage(url, html, res, userAgent, referer, depth, visited, signal) {
  ...
  chain = chain.then((result) => {
    ...
    if (!isVoeHost(url)) {
      ...
      if (voeMirrorUrl) {
        console.log(`Ennovelas unpacker: recognised ${getHostname(url)} as a VOE mirror by payload`);
        return voeMirrorUrl;
      }
    }
    return null;
  ...
  chain = chain.then((result) => {
    ...
    if (redirectUrl && redirectUrl !== url && isHttpUrl(redirectUrl)) {
      console.log(`Ennovelas unpacker: following JS redirect to ${redirectUrl}`);
  ...
  chain = chain.then((result) => {
    ...
    if (assignedRedirectUrl && assignedRedirectUrl !== url && isHttpUrl(assignedRedirectUrl)) {
      console.log(`Ennovelas unpacker: following assigned redirect to ${assignedRedirectUrl}`);
...
const ENNOVELAS_BASE_URL = "https://l.ennovelas-tv.com";
const ENNOVELAS_SEARCH_TIMEOUT_MS = 4500;
const ENNOVELAS_PAGE_TIMEOUT_MS = 5500;
const ENNOVELAS_PLAYER_CONCURRENCY = 3;
const MAX_DIRECT_PROBES = 5;
function ennovelasBrowserHeaders(userAgent, extra = {}) {
...
function ennovelasSlugifyTitle(value) {
...
function titleWithoutEpisode(value) {
  return String(value || "").replace(/\s+(?:cap[ií]tulo|ep)\s*\d+[\s\S]*$/i, "").trim();
}
...
function scoreEpisodeCandidate(result, query, originalTitle, extraTitles = [], season = null) {
  ...
  const slugText = (_b = (_a = result.url.match(/\/([^/?#]+)\/?$/)) == null ? void 0 : _a[1]) == null ? void 0 : _b.replace(/-/g, " ");
...
function episodeUrlCandidates(slug, episode) {
  return [
    `${ENNOVELAS_BASE_URL}/${slug}-capitulo-${episode}/`,
    `${ENNOVELAS_BASE_URL}/${slug}-capitulo-${episode}-final/`
  ];
}
...
function isEpisodePage(html, episode) {
  ...
  return new RegExp(`(?:cap[i\xED]tulo|ep)\\s*${Number(episode)}\\b`, "i").test(text) && /novel\.php\?post=|\/emb\/\?vid=|twitter:player/i.test(text);
}
function fetchPageOk(url, userAgent, signal, episode) {
  return fetchTextWithTimeout(url, { headers: ennovelasBrowserHeaders(userAgent), signal }, ENNOVELAS_PAGE_TIMEOUT_MS).then(({ res, text }) => {
...
function extractEpisodeResults(html) {
  ...
  $("article, .post, .item, .ml-item").each((_, el) => {
    ...
    const url = normalizeUrl(href, ENNOVELAS_BASE_URL);
    if (!url || seen.has(url)) return;
    const title = ($(el).find("h1,h2,h3,h4,.entry-title,.title").first().text() || $(el).find("img").attr("alt") || $(el).find('a[href*="capitulo"],a[href*="episodio"]').first().attr("title") || $(el).find('a[href*="capitulo"],a[href*="episodio"]').first().text()).trim().replace(/\s+/g, " ");
    if (title) {
      seen.add(url);
  ...
  $('a[href*="capitulo"],a[href*="episodio"]').each((_, el) => {
    const url = normalizeUrl($(el).attr("href"), ENNOVELAS_BASE_URL);
    if (!url || seen.has(url)) return;
    const title = ($(el).attr("title") || $(el).find("img").attr("alt") || $(el).text() || "").trim().replace(/\s+/g, " ");
...
function findEpisodeUrl(title, originalTitle, season, episode, userAgent, signal, extraTitles = []) {
  ...
  for (const baseTitle of [title, originalTitle, ...extraTitles]) {
    seasonStems.push(...seasonSlugStems(ennovelasSlugifyTitle(baseTitle), season));
  }
  for (const query of queries) plainStems.push(ennovelasSlugifyTitle(query));
  const orderedStems = season > 1 ? [...seasonStems, ...plainStems] : [...plainStems, ...seasonStems];
  const stems = [];
  ...
  const candidateUrls = [].concat(...stems.map((stem) => episodeUrlCandidates(stem, episode)));
  const orderedUrls = [
    ...candidateUrls.filter((url) => !url.endsWith("-final/")),
    ...candidateUrls.filter((url) => url.endsWith("-final/"))
  ];
  function tryDirect(i, probesLeft) {
    if (i >= orderedUrls.length || probesLeft <= 0) return null;
    return fetchPageOk(orderedUrls[i], userAgent, signal, episode).then((html) => {
      if (html) return { url: orderedUrls[i], html };
      return tryDirect(i + 1, probesLeft - 1);
    });
  }
  function runQuery(query) {
    const searchUrl = `${ENNOVELAS_BASE_URL}/?s=${encodeURIComponent(query)}`;
    return fetchTextWithTimeout(searchUrl, { headers: ennovelasBrowserHeaders(userAgent), signal }, ENNOVELAS_SEARCH_TIMEOUT_MS).then(({ res, text: html }) => {
      if (!res.ok) return null;
      let bestMatch = null;
      let bestScore = 0;
      ...
      return (bestMatch == null ? void 0 : bestMatch.url) || null;
    }).catch((error) => {
...
function extractPostWrapperUrls(value) {
  ...
  try {
    const decoded = base64ToUtf8String(payload);
...
function normalizePlayerUrl(url) {
  ...
  try {
    ...
    const watchMatch = /(^|\.)vk\.com$/i.test(parsed.hostname) && parsed.pathname.match(/^\/video(-?\d+)_(\d+)/);
    if (watchMatch) {
...
function isPlayableCandidate(url) {
  try {
    ...
    return /^\/(?:embed-|e|f|v|video_ext\.php)/i.test(parsed.pathname) || /\/embed/i.test(parsed.pathname) || /\.(?:m3u8|mp4|mkv)$/i.test(parsed.pathname);
  } catch (e) {
    return false;
  }
}
...
function scrape(title, originalTitle, year, type, season, episode, options = {}) {
  ...
  return findEpisodeUrl(title, originalTitle, season, episode, userAgent, signal, extraTitles).then((episodeMatch) => {
    if (!episodeMatch) {
      console.log(`Ennovelas: No episode found for "${title}" episode ${episode}`);
      return [];
    }
    const episodeHtmlPromise = episodeMatch.html ? Promise.resolve(episodeMatch.html) : fetchPageOk(episodeMatch.url, userAgent, signal, episode);
    return episodeHtmlPromise.then((episodeHtml) => {
      ...
      return mapWithConcurrency(
        playerUrls,
        ENNOVELAS_PLAYER_CONCURRENCY,
        (playerUrl) => resolvePlayerStream(playerUrl, userAgent, episodeMatch.url, { signal }).then((resolvedUrl) => {
...
const MEDIAFLOW_PROXY_API_PASSWORD = ((typeof process !== "undefined" && process.env ? process.env.MEDIAFLOW_PROXY_API_PASSWORD : undefined));
...
function finalizeStreams(streams) {
  return (streams || []).filter(Boolean).map((stream, index) => ({ stream, index })).sort((a, b) => {
...
function getStreams(tmdbId, mediaType, seasonNum, episodeNum) {
  ...
  return Promise.all([fetchTmdbDetails(tmdbId, mediaType), getAlternativeTitles(mediaType, tmdbId)]).then(([details, extraTitles]) => {
    ...
    console.error("Ennovelas (Nuvio): getStreams failed:", error && error.message);
    return [];
  });
}
