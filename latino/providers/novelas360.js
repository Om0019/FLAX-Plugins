// Built from src/providers/novelas360.js for the Hermes/es2016 runtime Nuvio's local-scraper
// sandbox targets -- do not hand-edit. Regenerate with:
//   npx esbuild@0.28.1 --target=es2016 --format=cjs --platform=neutral src/providers/novelas360.js > latino/providers/novelas360.js
// Edit src/providers/novelas360.js instead, then rebuild.
...
const NOVELAS360_SEARCH_TIMEOUT_MS = 5e3;
...
function titleWithoutEpisode(value) {
  return String(value || "").replace(/\s+cap[ií]tulo\s*\d+[\s\S]*$/i, "").trim();
}
function scoreEpisodeCandidate(result, query, originalTitle, extraTitles = []) {
  ...
  const cleanSlugTitle = cleanText(titleWithoutEpisode(
    (_b = (_a = result.url.match(/\/video\/([^/?#]+)/)) == null ? void 0 : _a[1]) == null ? void 0 : _b.replace(/-/g, " ")
  ));
...
function extractEpisodeResults(html) {
  ...
  $("article, .post, .video").each((_, el) => {
    ...
    if (!url || seen.has(url)) return;
    seen.add(url);
    const title = ($(el).find("h1,h2,h3,h4,.entry-title,.post-title").first().text() || $(el).find("img").attr("alt") || $(el).find('a[href*="/video/"]').text()).trim().replace(/\s+/g, " ");
    if (title) results.push({ url, title });
  });
  $('a[href*="/video/"]').each((_, el) => {
    const url = normalizeUrl($(el).attr("href"), NOVELAS360_BASE_URL);
    if (!url || seen.has(url)) return;
    const text = ($(el).text() || $(el).attr("title") || $(el).find("img").attr("alt") || "").trim().replace(/\s+/g, " ");
...
function isPlayableCandidate(url) {
  try {
    ...
    return /\/player\/embed_player\.php$/i.test(path) || /^\/(?:e|f|v)\/[^/]+/i.test(path) || /\.(?:m3u8|mp4|mkv)$/i.test(path);
  } catch (e) {
    return false;
  }
}
...
function fetchPageOk(url, userAgent, signal) {
  return fetchTextWithTimeout(url, { headers: novelas360BrowserHeaders(userAgent), signal }, NOVELAS360_PAGE_TIMEOUT_MS).then(({ res, text }) => {
...
function findEpisodeUrl(title, originalTitle, season, episode, userAgent, signal, extraTitles = []) {
  ...
  function runQuery(query) {
    ...
    return fetchTextWithTimeout(searchUrl, { headers: novelas360BrowserHeaders(userAgent), signal }, NOVELAS360_SEARCH_TIMEOUT_MS).then(({ res, text: html }) => {
      if (!res.ok) return null;
      let bestMatch = null;
      let bestScore = 0;
...
function scrape(title, originalTitle, year, type, season, episode, options = {}) {
  ...
  return findEpisodeUrl(title, originalTitle, season, episode, userAgent, signal, extraTitles).then((episodeMatch) => {
    ...
    const episodeHtmlPromise = episodeMatch.html ? Promise.resolve(episodeMatch.html) : fetchPageOk(episodeMatch.url, userAgent, signal);
