...
const CUEVANA_PAGE_TIMEOUT_MS = 5e3;
...
function slugifyTitle(str, options = {}) {
  ...
  return str.toLowerCase().normalize("NFD").replace(/[̀-ͯ]/g, "").replace(/&/g, ` ${ampersandWord} `).replace(/\band\b/g, ampersandWord).replace(/\by\b/g, ampersandWord).replace(/['’:.!,?()[\]/]+/g, " ").replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
}
...
function resolveWithTimeout(url, userAgent, referer, signal) {
  return new Promise((resolve) => {
    ...
    resolvePlayerStream(url, userAgent, referer, { signal }).then(finish).catch(() => finish(null));
  });
}
...
function probePage(candidate, userAgent, signal) {
  return fetchWithTimeout(candidate.url, { headers: { "User-Agent": userAgent }, signal }, CUEVANA_PROBE_TIMEOUT_MS).then((res) => res.ok ? candidate : null).catch(() => null);
}
function scrape(title, originalTitle, year, type, season, episode, options = {}) {
  ...
  return firstResultInOrder(candidates, PROBE_CONCURRENCY, (candidate) => probePage(candidate, userAgent, signal)).then((bestMatch) => {
    if (bestMatch) {
    ...
    const targetPageUrl = type === "series" ? `${bestMatch.url}/episodio-${season}x${episode}` : bestMatch.url;
    return fetchTextWithTimeout(targetPageUrl, { headers: { "User-Agent": userAgent }, signal }, CUEVANA_PAGE_TIMEOUT_MS).then(({ res: pageRes, text: pageHtml }) => {
      if (!pageRes.ok) {
      ...
      return mapWithConcurrency(wrapperUrls, PLAYER_CONCURRENCY, (wrapperUrl, index) => {
        ...
        return resolveWithTimeout(decodedUrl, userAgent, targetPageUrl, signal).then((directUrl) => {
          if (!directUrl) return null;
          return {
            name: "Cuevana3i",
            title: `\u{1F1F2}\u{1F1FD} ${optionName}`,
            url: directUrl,
