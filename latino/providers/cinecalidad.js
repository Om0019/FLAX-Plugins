...
const PLAYER_FAST_MIN_WAIT_MS = 1e3;
...
function normalizeServerName(label) {
  return (label || "").replace("Recomendado", "").replace(/Contraseña:.*/i, "").replace(/\s+/g, " ").trim();
}
...
function isPlayableDownloadTarget(url, userAgent, referer, signal) {
  return fetchWithTimeout(url, {
    ...
    signal
  }, VALIDATION_TIMEOUT_MS).then((res) => {
...
function resolveDownloadPage(downloadPageUrl, userAgent, referer, signal) {
  return fetchTextWithTimeout(downloadPageUrl, {
    ...
    signal
  }, CINECALIDAD_PAGE_TIMEOUT_MS).then(({ res, text: html }) => {
    if (!res.ok) return null;
...
function resolveDownloadUrl(url, userAgent, referer, options = {}) {
  ...
  }, PLAYER_FETCH_TIMEOUT_MS).then(({ res, text: html }) => {
...
function scrape(title, originalTitle, year, type, season, episode, options = {}) {
  ...
  return raceTitleSearches(racedTitles, performSearch).then((bestMatch) => {
    ...
    return fetchTextWithTimeout(targetPageUrl, {
      ...
      const playerPromises = sortedPlayerOptions.map(
        (opt) => resolvePlayerStream(opt.playerUrl, userAgent, targetPageUrl, { signal: playerController.signal }).then((directUrl) => {
          if (directUrl) {
            streams.push({
              name: "Cinecalidad",
              title: `\u{1F1F2}\u{1F1FD} ${opt.serverName}`,
              url: directUrl,
      ...
      return waitForPlayerResolvers(playerPromises, streams).then((playerCompletion) => {
        ...
        return Promise.all(
          downloadPageLinks.map(
            (downloadLink) => resolveDownloadPage(downloadLink.downloadPageUrl, userAgent, targetPageUrl, signal).then((pageUrl) => {
              if (!pageUrl) return null;
              return resolveDownloadUrl(pageUrl, userAgent, targetPageUrl, { signal }).then(
                (resolvedUrl) => isPlayableDownloadTarget(resolvedUrl, userAgent, targetPageUrl, signal).then((isPlayable) => {
          ...
          return mapWithConcurrency(
            ...
            (link) => resolveDownloadUrl(link.downloadUrl, userAgent, targetPageUrl, { signal }).then(
              (downloadUrl) => isPlayableDownloadTarget(downloadUrl, userAgent, targetPageUrl, signal).then((isPlayable) => {
                if (!isPlayable) return null;
                return {
                  name: "Cinecalidad",
