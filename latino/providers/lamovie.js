...
const LAMOVIE_SEARCH_TIMEOUT_MS = 8e3;
const LAMOVIE_PLAYER_TIMEOUT_MS = 5500;
const LAMOVIE_EPISODES_TIMEOUT_MS = 5e3;
...
function runLaMovieQuery(query, title, originalTitle, year, type, userAgent, signal, extraTitles) {
  ...
  return fetchJsonWithTimeout(url, {
    ...
    var _a;
    if (!res.ok) return null;
    const posts = Array.isArray((_a = data == null ? void 0 : data.data) == null ? void 0 : _a.posts) ? data.data.posts : [];
    let bestMatch = null;
    let bestScore = 0;
...
function getEpisodePostId(seriesId, season, episode, userAgent, signal) {
  ...
  return fetchJsonWithTimeout(url, {
    ...
    return (match == null ? void 0 : match._id) || null;
  }).catch((error) => {
...
function scrape(title, originalTitle, year, type, season, episode, options = {}) {
  ...
  return lamovieSearch(title, originalTitle, year, type, userAgent, signal, extraTitles).then((match) => {
    if (!match) {
      console.log(`LaMovie: No matching content found for "${title}"`);
      return [];
    }
    const postIdPromise = type === "series" ? getEpisodePostId(match._id, season, episode, userAgent, signal).then((postId) => {
    ...
    return postIdPromise.then((postId) => {
      ...
      return fetchJsonWithTimeout(playerUrl, {
        ...
        const embeds = Array.isArray((_a = playerData == null ? void 0 : playerData.data) == null ? void 0 : _a.embeds) ? playerData.data.embeds : [];
        ...
        return mapWithConcurrency(embeds, LAMOVIE_PLAYER_CONCURRENCY, (embed, index) => {
          ...
          return resolvePlayerStream(embedUrl, userAgent, LAMOVIE_BASE_URL, { signal }).then((resolvedUrl) => {
            ...
            return {
              name: "LaMovie",
              title: `\u{1F1F2}\u{1F1FD} ${embed.lang || "Latino"} ${embed.quality || embed.server || `Opcion ${index + 1}`}`.trim(),
              url: resolvedUrl,
...
function getStreams(tmdbId, mediaType, seasonNum, episodeNum) {
  ...
  return Promise.all([fetchTmdbDetails(tmdbId, mediaType), getAlternativeTitles(mediaType, tmdbId)]).then(([details, extraTitles]) => {
    ...
    console.error("Plugin getStreams failed:", error && error.message);
    return [];
  });
}
